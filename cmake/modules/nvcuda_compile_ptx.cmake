...
FUNCTION(NVCUDA_COMPILE_PTX)
  ...
  FOREACH(input ${NVCUDA_COMPILE_PTX_SOURCES})
    # message("INPUT: ${input}")
    # message("DEPS: ${NVCUDA_COMPILE_PTX_DEPENDENCIES}")
    ...
    # message(" ${CUDA_NVCC_EXECUTABLE} --machine=64 --ptx ${NVCUDA_COMPILE_PTX_NVCC_OPTIONS}  -o ${output} ${input}")

    add_custom_command(
      OUTPUT  "${output}"
      DEPENDS "${input}" ${NVCUDA_COMPILE_PTX_DEPENDENCIES}
      COMMAND ${CUDA_NVCC_EXECUTABLE} --machine=64 --ptx ${NVCUDA_COMPILE_PTX_NVCC_OPTIONS}  -o "${output}" "${input}"
