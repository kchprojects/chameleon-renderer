...
// # define PING std::cout << __FILE__ << "::" << __LINE__ << ": " <<
...
using std::min;
// inline __both__ double sin(double f) { return ::sin(f); }
inline __both__ float saturate(const float& f) {
...
// inline __both__ float abs(float f)      { return fabsf(f); }
// inline __both__ double abs(double f)    { return fabs(f); }
inline __both__ float rcp(float f) {
    return 1.f / f;
}
inline __both__ double rcp(double d) {
    return 1. / d;
}
...
inline __both__ float sqrt(const float f) {
    return ::sqrtf(f);
}
inline __both__ double sqrt(const double d) {
    return ::sqrt(d);
}
}  // namespace overloaded
// #endif
...
inline std::string prettyNumber(const size_t s) {
    ...
    if (s >= (1024LL * 1024LL * 1024LL * 1024LL)) {
        osp_snprintf(buf, 1000, "%.2fT",
                     s / (1024.f * 1024.f * 1024.f * 1024.f));
