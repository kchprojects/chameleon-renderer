...
void Scene::set_lights(std::vector<vec3f> lights) {
...
const chameleon::ModifyGuard<std::vector<gdt::vec3f>>& Scene::light_positions()
    const {
    return _light_positions;
}
chameleon::ModifyGuard<std::vector<gdt::vec3f>>& Scene::light_positions() {
    return _light_positions;
}
