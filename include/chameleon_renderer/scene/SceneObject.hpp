...
struct ModifyGuard{
    ModifyGuard() = default;
    ModifyGuard( T obj) : _obj(std::move(obj)) {}
    ModifyGuard(const ModifyGuard<T>& other):_obj(other._obj){}
    ModifyGuard(ModifyGuard<T>&& other):_obj(std::move(other._obj)){}
...
struct SceneObject {
    ...
    SceneObject() = default;
    SceneObject( mat_t obj_mat) : _object_matrix(std::move(obj_mat)) {}
    ...
    bool is_modified()const{
        return _object_matrix.is_modified();
    }
