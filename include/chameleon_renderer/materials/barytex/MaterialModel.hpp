...
namespace chameleon {
enum class MaterialModel { NoModel = 0,Lambert, BlinPhong, CookTorrance };
...
#endif
            T
            compute(const glm::vec3& normal, const glm::vec3& light,
                    const glm::vec3& view,
...
struct ModelReflectance<MaterialModel::CookTorrance, T> {
    ...
    static
...
#endif
            T
            compute(const glm::vec3& N, const glm::vec3& L, const glm::vec3& V,
        ...
        T out = NL * data.albedo + data.Ks * D(data.m, alpha) *
                                       ...
                                       (T(4) * NL);
        // std::cout << "data: " << data.albedo << "," << data.Ks << "," <<
        // data.F0
        //           << "," << data.m << std::endl;
